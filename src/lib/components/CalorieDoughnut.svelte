	...
	import type { Plugin } from 'chart.js';
			...
			const centerTextPlugin: Plugin = {
				...
				beforeDraw(chart) {
					...
					const opts = (chart.options.plugins as Record<string, Record<string, string> | undefined>)?.centerText;
					if (!opts) return;
			...
			function buildData(e: number, g: number) {
				const over = e > g;
				return over
					? { labels: ['Over'], datasets: [{ data: [e], backgroundColor: ['#ef4444'], borderWidth: 0 }] }
					: {
			...
			function buildOptions(e: number, g: number) {
				const over = e > g;
				const subtext = over ? `${e - g} over` : `of ${g} kcal`;
				return {
					responsive: true,
